...
public class WelcomeActivity extends XposedBaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mAdapter.add(new WelcomeItem(R.string.tabInstall,
				R.string.tabInstallDescription));
		mAdapter.add(new WelcomeItem(R.string.tabModules,
				R.string.tabModulesDescription));
		mAdapter.add(new WelcomeItem(R.string.tabDownload,
				R.string.tabDownloadDescription));
		mAdapter.add(new WelcomeItem(R.string.tabSettings,
				R.string.tabSettingsDescription));
		mAdapter.add(new WelcomeItem(R.string.tabSupport,
				R.string.tabSupportDescription));
		mAdapter.add(new WelcomeItem(R.string.tabLogs,
				R.string.tabLogsDescription));
		mAdapter.add(new WelcomeItem(R.string.tabAbout,
				R.string.tabAboutDescription));
	...
	class WelcomeAdapter extends ArrayAdapter<WelcomeItem> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (position == XposedInstallerActivity.TAB_INSTALL) {
				xposedActive = XposedApp.getActiveXposedVersion() >= InstallerFragment
						.getJarLatestVersion();
			} else if (position == XposedInstallerActivity.TAB_DOWNLOAD) {
				frameworkUpdateVersion = mRepoLoader
						.getFrameworkUpdateVersion();
				moduleUpdateAvailable = mRepoLoader.hasModuleUpdates();

				if (frameworkUpdateVersion != null) {
					((TextView) view
							.findViewById(R.id.txtFrameworkUpdateAvailable))
							.setText(getResources()
									.getString(
											R.string.welcome_framework_update_available,
											(String) frameworkUpdateVersion));
				}
			}

			view.findViewById(R.id.txtXposedNotActive).setVisibility(
					!xposedActive ? View.VISIBLE : View.GONE);
			view.findViewById(R.id.txtFrameworkUpdateAvailable).setVisibility(
					frameworkUpdateVersion != null ? View.VISIBLE : View.GONE);
			view.findViewById(R.id.txtUpdateAvailable).setVisibility(
					moduleUpdateAvailable ? View.VISIBLE : View.GONE);
