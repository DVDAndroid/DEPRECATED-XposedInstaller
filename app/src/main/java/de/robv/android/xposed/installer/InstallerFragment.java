...
public class InstallerFragment extends Fragment {
	...
	public static int getJarLatestVersion() {
		if (JAR_LATEST_VERSION == -1) {
			try {
				JAR_LATEST_VERSION = getJarVersion(XposedApp.getInstance()
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (activity instanceof XposedDropdownNavActivity)
			((XposedDropdownNavActivity) activity)
					.setNavItem(XposedDropdownNavActivity.TAB_INSTALL);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		txtAppProcessInstalledVersion = (TextView) v
				.findViewById(R.id.app_process_installed_version);
		txtAppProcessLatestVersion = (TextView) v
				.findViewById(R.id.app_process_latest_version);
		txtJarInstalledVersion = (TextView) v
				.findViewById(R.id.jar_installed_version);
		txtJarLatestVersion = (TextView) v
				.findViewById(R.id.jar_latest_version);
		...
		btnInstallMode.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra(XposedInstallerActivity.EXTRA_SECTION,
						XposedDropdownNavActivity.TAB_SETTINGS);
		 ...
		 * txtInstallError.setText(String
		 * .format(getString(R.string.not_tested_but_compatible),
		 * Build.VERSION.SDK_INT)); txtInstallError.setVisibility(View.VISIBLE);
		 * } }
		 */
		 ...
		 * being displayed
		 * getView().findViewById(R.id.install_reverted_warning).
		 * setVisibility(View.GONE); } }); } }); } else { String errorText =
		 ...
		 * XposedApp
		 * .getPreferences().edit().remove(PREF_LAST_SEEN_BINARY).commit(); } //
		 * Dismiss any warning already being displayed
		 * getView().findViewById(R.id
		 * .install_reverted_warning).setVisibility(View.GONE); } }); } });
		 */
		...
		btnReboot.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				areYouSure(R.string.reboot, new AsyncDialogClickListener(
						btnReboot.getText()) {
					@Override
		...
		if (!XposedApp.getPreferences().getBoolean("hide_install_warning",
				false)) {
			final View dontShowAgainView = inflater.inflate(
					R.layout.dialog_install_warning, null);
		 ...
		 */
		int lastSeenBinary = XposedApp.getPreferences().getInt(
				PREF_LAST_SEEN_BINARY, Integer.MIN_VALUE);
		if (lastSeenBinary != Integer.MIN_VALUE) {
			final View vInstallRevertedWarning = v
					.findViewById(R.id.install_reverted_warning);
			final TextView txtInstallRevertedWarning = (TextView) v
					.findViewById(R.id.install_reverted_warning_text);
			vInstallRevertedWarning
					.setOnClickListener(new View.OnClickListener() {
						@Override
						public void onClick(View v) {
							// Stop tracking and dismiss the info panel
							XposedApp.getPreferences().edit()
									.remove(PREF_LAST_SEEN_BINARY).commit();
			...
			if (lastSeenBinary < 0 && appProcessInstalledVersion > 0) {
				...
				// reappeared
				txtInstallRevertedWarning.setText(getString(
						R.string.uninstall_reverted,
				...
				txtInstallRevertedWarning.setText(getString(
						R.string.install_reverted,
						versionToText(lastSeenBinary),
						versionToText(appProcessInstalledVersion)));
				vInstallRevertedWarning.setVisibility(View.VISIBLE);
			} else if (appProcessInstalledVersion > lastSeenBinary) {
				...
				XposedApp
						.getPreferences()
						.edit()
						.putInt(PREF_LAST_SEEN_BINARY,
								appProcessInstalledVersion).commit();
			} else {
				// All is ok
			}
		}
	...
	private void refreshVersions() {
		...
		txtAppProcessInstalledVersion
				.setText(versionToText(appProcessInstalledVersion));
		txtAppProcessLatestVersion
				.setText(versionToText(appProcessLatestVersion));
		...
		if (appProcessInstalledVersion < appProcessLatestVersion)
			txtAppProcessInstalledVersion.setTextColor(getResources().getColor(
					R.color.warning));
		else
			txtAppProcessInstalledVersion.setTextColor(getResources().getColor(
		...
		if (jarInstalledVersion < jarLatestVersion)
			txtJarInstalledVersion.setTextColor(getResources().getColor(
					R.color.warning));
		else
			txtJarInstalledVersion.setTextColor(getResources().getColor(
					R.color.darker_green));
	...
	private String versionToText(int version) {
		return (version == 0) ? getString(R.string.none) : Integer
				.toString(version);
	}
	...
	private void refreshKnownIssue() {
		...
		if (issueName != null) {
			...
			txtKnownIssue.setText(getString(R.string.install_known_issue,
					issueName));
			...
			if (btnInstall.isEnabled())
				btnInstall.setTextColor(getResources()
			...
			txtInstallError.setTextColor(getResources().getColor(
					R.color.warning));
		}
	}
	...
	private int getInstalledAppProcessVersion() {
		try {
			String filename = "/system/bin/"
					+ ((Build.VERSION.SDK_INT >= 21) ? "app_process32_xposed"
							: "app_process");
	...
	private int getLatestAppProcessVersion() {
		...
		try {
			return getAppProcessVersion(getActivity().getAssets().open(
					APP_PROCESS_NAME));
		} catch (Exception e) {
			return 0;
		}
	...
	private boolean install() {
		...
		try {
			messages.add(getString(R.string.sdcard_location, XposedApp
					.getInstance().getExternalFilesDir(null)));
			messages.add("");
			...
			if (installMode == INSTALL_MODE_NORMAL)
				...
				offerRebootToRecovery(messages,
						"Xposed-Installer-Recovery.zip", installMode);
	...
	private void offerRebootToRecovery(List<String> messages,
			final String file, final int installMode) {
				...
				new AsyncDialogClickListener(getString(R.string.reboot)) {
					...
					protected void onAsyncClick(DialogInterface dialog,
						...
						if (installMode == INSTALL_MODE_RECOVERY_AUTO) {
							...
							mRootUtil.executeWithBusybox("rm /cache/recovery/"
									+ file, null);
							AssetUtil.removeBusybox();
						}
					}
