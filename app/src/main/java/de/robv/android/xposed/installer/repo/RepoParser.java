...
public class RepoParser {
	...
	public static Spanned parseSimpleHtml(final Context c, String source,
		...
		Spanned html = Html.fromHtml(source, new Html.ImageGetter() {
			...
			public Drawable getDrawable(String source) {
				...
				Drawable empty = c.getResources().getDrawable(
	...
	protected void readRepo() throws XmlPullParserException, IOException {
		...
		repository.isPartial = "true".equals(parser.getAttributeValue(NS,
				"partial"));
	...
	protected String readRemoveModule() throws XmlPullParserException,
			IOException {
	...
	protected void skip(boolean showWarning) throws XmlPullParserException,
			IOException {
		parser.require(XmlPullParser.START_TAG, null, null);
		if (showWarning)
			Log.w(TAG,
					"skipping unknown/erronous tag: "
	...
	protected void leave(int targetDepth) throws XmlPullParserException,
			IOException {
		Log.w(TAG,
				"leaving up to level " + targetDepth + ": "
						+ parser.getPositionDescription());
	...
	static class ImageGetterAsyncTask extends AsyncTask<TextView, Void, Bitmap> {
		...
		protected void onPostExecute(final Bitmap bitmap) {
			try {
				...
				levelListDrawable.setBounds(0, 0, bitmap.getWidth()
						* multiplier, bitmap.getHeight() * multiplier);
