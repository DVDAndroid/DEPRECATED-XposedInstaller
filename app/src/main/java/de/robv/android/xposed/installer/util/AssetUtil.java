...
public class AssetUtil {
	public static final File BUSYBOX_FILE = new File(XposedApp.getInstance()
			.getCacheDir(), "busybox-xposed");
	...
	public static File writeAssetToCacheFile(String assetName, String fileName,
			int mode) {
		return writeAssetToFile(assetName, new File(XposedApp.getInstance()
				.getCacheDir(), fileName), mode);
	}
	...
	public synchronized static void extractBusybox() {
		...
		if (isStaticBusyboxAvailable()) {
			try {
				...
				Log.e(XposedApp.TAG, "could not load assets from "
