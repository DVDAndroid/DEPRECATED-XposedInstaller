...
public class DownloadDetailsVersionsFragment extends ListFragment {
			...
			if (!repoLoader.isVersionShown(module.versions.get(0))) {
				...
				txtHeader.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						mActivity
								.gotoPage(DownloadDetailsActivity.DOWNLOAD_SETTINGS);
					}
				});
	...
	private static class DownloadModuleCallback implements
	...
	private class VersionsAdapter extends ArrayAdapter<ModuleVersion> {
		...
		public VersionsAdapter(Context context, InstalledModule installed) {
			...
			mColorUpdateAvailable = getResources().getColor(
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			holder.txtRelType
					.setTextColor(item.relType == ReleaseType.STABLE ? mColorRelTypeStable
							: mColorRelTypeOthers);

			if (item.uploaded > 0) {
				holder.txtUploadDate.setText(mDateFormatter.format(new Date(
						item.uploaded)));
			...
			holder.downloadView
					.setDownloadFinishedCallback(new DownloadModuleCallback(
							item));
			...
			if (item.changelog != null && !item.changelog.isEmpty()) {
				...
				if (item.changelogIsHtml) {
					holder.txtChanges.setText(RepoParser
							.parseSimpleHtml(item.changelog));
					holder.txtChanges.setMovementMethod(LinkMovementMethod
							.getInstance());
				} else {
