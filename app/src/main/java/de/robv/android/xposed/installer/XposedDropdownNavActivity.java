...
public abstract class XposedDropdownNavActivity extends XposedBaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (navigationItemList == null) {
			...
			navigationItemList.add(makeNavigationItem(
					getString(R.string.tabInstall), InstallerFragment.class));
			navigationItemList.add(makeNavigationItem(
					getString(R.string.tabModules), ModulesFragment.class));
			navigationItemList.add(makeNavigationItem(
					getString(R.string.tabDownload), DownloadFragment.class));
			navigationItemList.add(makeNavigationItem(
					getString(R.string.tabSettings), SettingsFragment.class));
			navigationItemList.add(makeNavigationItem(
					getString(R.string.tabSupport), SupportFragment.class));
			navigationItemList.add(makeNavigationItem(
					getString(R.string.tabLogs), LogsFragment.class));
			navigationItemList.add(makeNavigationItem(
					getString(R.string.tabAbout), AboutFragment.class));
		}

		SimpleAdapter adapter = new SimpleAdapter(getActionBar()
				.getThemedContext(), navigationItemList,
		...
		bar.setListNavigationCallbacks(adapter, new OnNavigationListener() {
			...
			public boolean onNavigationItemSelected(int itemPosition,
				...
				if (navigateViaIntent()) {
					Intent intent = new Intent(XposedDropdownNavActivity.this,
				...
				Fragment fragment = Fragment.instantiate(
						XposedDropdownNavActivity.this, fragmentClass);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == android.R.id.home) {
			if (!getIntent().getBooleanExtra(NavUtil.FINISH_ON_UP_NAVIGATION,
					false)) {
