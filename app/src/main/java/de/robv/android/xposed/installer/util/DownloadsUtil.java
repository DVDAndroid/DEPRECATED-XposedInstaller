...
public class DownloadsUtil {
	...
	private static final SharedPreferences mPref = mApp.getSharedPreferences(
			"download_cache", Context.MODE_PRIVATE);
	...
	public static List<DownloadInfo> getAllForUrl(Context context, String url) {
		...
		while (c.moveToNext()) {
			...
			downloads.add(new DownloadInfo(c.getLong(columnId), c
					.getString(columnUri), c.getString(columnTitle), c
					.getLong(columnLastMod), localFilename, c
					.getInt(columnStatus), c.getInt(columnTotalSize), c
					.getInt(columnBytesDownloaded), c.getInt(columnReason)));
		}
		...
		try {
			...
			if (connection instanceof HttpURLConnection) {
				...
				if (useNotModifiedTags) {
					String modified = mPref.getString("download_" + url
							+ "_modified", null);
