...
public final class RepoDb extends SQLiteOpenHelper {
	...
	private RepoDb(Context context) {
		super(context, new File(context.getCacheDir(),
				RepoDbDefinitions.DATABASE_NAME).getPath(), null,
				RepoDbDefinitions.DATABASE_VERSION);
	}

	public synchronized static void init(Context context, RepoLoader repoLoader) {
		if (mInstance != null)
			throw new IllegalStateException(RepoDb.class.getSimpleName()
	...
	public static Map<Long, Repository> getRepositories() {
		...
		while (c.moveToNext()) {
			...
			long id = c.getLong(c
					.getColumnIndexOrThrow(RepositoriesColumns._ID));
			repo.url = c.getString(c
					.getColumnIndexOrThrow(RepositoriesColumns.URL));
			repo.name = c.getString(c
					.getColumnIndexOrThrow(RepositoriesColumns.TITLE));
			repo.partialUrl = c.getString(c
					.getColumnIndexOrThrow(RepositoriesColumns.PARTIAL_URL));
			repo.version = c.getString(c
					.getColumnIndexOrThrow(RepositoriesColumns.VERSION));
	...
	private static long insertModuleVersion(long moduleId, ModuleVersion version) {
		...
		return mDb
				.insertOrThrow(ModuleVersionsColumns.TABLE_NAME, null, values);
	}
	...
	public static void deleteModule(long repoId, String packageName) {
		mDb.delete(ModulesColumns.TABLE_NAME, ModulesColumns.REPO_ID
				+ " = ? AND " + ModulesColumns.PKGNAME + " = ?", new String[] {
				Long.toString(repoId), packageName });
	}
	...
	public static Module getModuleByPackageName(String packageName) {
		...
		mod.packageName = c.getString(c
				.getColumnIndexOrThrow(ModulesColumns.PKGNAME));
		mod.name = c.getString(c.getColumnIndexOrThrow(ModulesColumns.TITLE));
		mod.summary = c.getString(c
				.getColumnIndexOrThrow(ModulesColumns.SUMMARY));
		mod.description = c.getString(c
				.getColumnIndexOrThrow(ModulesColumns.DESCRIPTION));
		...
		mod.support = c.getString(c
				.getColumnIndexOrThrow(ModulesColumns.SUPPORT));
		mod.created = c
		...
		projection = new String[] { ModuleVersionsColumns.NAME,
				...
				ModuleVersionsColumns.RELTYPE, ModuleVersionsColumns.UPLOADED, };
		...
		while (c.moveToNext()) {
			...
			version.name = c.getString(c
					.getColumnIndexOrThrow(ModuleVersionsColumns.NAME));
			version.code = c.getInt(c
					.getColumnIndexOrThrow(ModuleVersionsColumns.CODE));
			version.downloadLink = c
					.getString(c
							.getColumnIndexOrThrow(ModuleVersionsColumns.DOWNLOAD_LINK));
			version.md5sum = c.getString(c
					.getColumnIndexOrThrow(ModuleVersionsColumns.MD5SUM));
			version.changelog = c.getString(c
					.getColumnIndexOrThrow(ModuleVersionsColumns.CHANGELOG));
			version.changelogIsHtml = c
					.getInt(c
							.getColumnIndexOrThrow(ModuleVersionsColumns.CHANGELOG_IS_HTML)) > 0;
			version.relType = ReleaseType.fromOrdinal(c.getInt(c
					.getColumnIndexOrThrow(ModuleVersionsColumns.RELTYPE)));
			version.uploaded = c.getLong(c
					.getColumnIndexOrThrow(ModuleVersionsColumns.UPLOADED));
		...
		while (c.moveToNext()) {
			String label = c.getString(c
					.getColumnIndexOrThrow(MoreInfoColumns.LABEL));
			String value = c.getString(c
					.getColumnIndexOrThrow(MoreInfoColumns.VALUE));
	...
	public static void updateModuleLatestVersion(String packageName) {
		int maxShownReleaseType = mRepoLoader.getMaxShownReleaseType(
				packageName).ordinal();
		mDb.execSQL("UPDATE " + ModulesColumns.TABLE_NAME + " SET "
				+ ModulesColumns.LATEST_VERSION + " = (SELECT "
				+ ModuleVersionsColumns._ID + " FROM "
				+ ModuleVersionsColumns.TABLE_NAME + " AS v" + " WHERE v."
				+ ModuleVersionsColumns.MODULE_ID + " = "
				+ ModulesColumns.TABLE_NAME + "." + ModulesColumns._ID
				+ " AND reltype <= ? LIMIT 1)" + " WHERE "
				+ ModulesColumns.PKGNAME + " = ?", new Object[] {
				maxShownReleaseType, packageName });
	}
	...
	public static Cursor queryModuleOverview(int sortingOrder,
		...
		// Query
		Cursor c = mDb.query(ModulesColumns.TABLE_NAME + " AS m"
				+ " LEFT JOIN " + ModuleVersionsColumns.TABLE_NAME + " AS v"
				+ " ON v." + ModuleVersionsColumns._ID + " = m."
				+ ModulesColumns.LATEST_VERSION + " LEFT JOIN "
				+ InstalledModulesColumns.TABLE_NAME + " AS i" + " ON i."
				+ InstalledModulesColumns.PKGNAME + " = m."
				+ ModulesColumns.PKGNAME, projection, where, whereArgs, null,
				null, sbOrder.toString());
	...
	private static String getFirstUpdate(boolean framework) {
		...
		String[] whereArgs = new String[] { ModuleUtil.getInstance()
		...
		if (c.moveToFirst())
			latestVersion = c
					.getString(c
							.getColumnIndexOrThrow(InstalledModulesUpdatesColumns.LATEST_NAME));
		c.close();
